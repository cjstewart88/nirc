    ...
    self.addListener("raw", function (message) { // {{{
        ...
        switch ( message.command ) {
            ...
            case "JOIN":
                ...
                messageToEmit.message   = (self.nick == message.nick ? 'Now talking in ' + message.args[0] : '* Joins: '+ message.nick );

                if (self.nick == message.nick) {
                    self.chanData(message.args[0], true);
                    
                    self.emit('successfullyJoinedChannel', message.args[0]);
                }
                else {
            ...
            case "PART":
                ...
                if (self.nick == message.nick) {
                    ...
                    self.emit('successfullyPartedChannel', channel);
                }
                else {
                    var channel = self.chanData(message.args[0]);
                    delete channel.users[message.nick];
        ...
        self.conn = tls.connect(self.opt.port, self.opt.server, creds, function() {
           ...
           if (self.conn.authorized ||
              ...
              self.conn.setEncoding('utf-8');
                ...
                self.send.apply(self, ['JOIN'].concat(self.opt.channels));
                self.emit('connect');
           } else {
